...
A Chrome Manifest V3 extension: **click manually** on a YouTube watch page to record. It captures the full tab surface plus page audio, crops the **player area (or any region you drag-select)** frame by frame with Canvas inside the browser, and finally exports a **video file with audio** to your downloads folder — natively prefers **MP4 (H.264/AAC)**, automatically falling back to **WebM** when the browser or OS cannot provide it.
...
- **Page hotkey**: the default single key `R` starts recording when idle and stops/saves while recording. Change it freely in the popup → **Settings** (`Ctrl` / `Alt` / `Shift` / `Command` combinations supported). It only works on YouTube pages, never steals keys from other tabs, and never conflicts with global browser shortcuts.
...
├── background.js            Service Worker: offscreen lifecycle, message routing, global state & badge, download proxy
...
├── content/
│   ├── guard.js             Page lock mask used during recording (hole cut to the picture rect; exists only during a session)
│   ├── selector.js          Region-select overlay (enabled on demand after clicking "Region record")
│   ├── content.js           Headless script: player rect heartbeat + hidden/navigation notices + mask toggle
...
├── popup.js
├── popup/
│   └── settings.js          Settings modal (hotkey toggle, key capture, conflict warnings)
...
1. The **content script** (injects no UI of its own; it only reports data) locates the player every ~120 ms and reports the true picture rectangle plus the viewport basis (CSS size, `devicePixelRatio`, visual-viewport offset).
...
1. Open any **YouTube watch page** (public videos; DRM / members-only videos record as a black frame — a browser protection restriction).
...
4. Click **Stop and save** → the file is assembled and downloaded as `YouTube-YYYYMMDD-HHMMSS.mp4` (`.webm` on fallback environments).
5. **Hotkey**: press the default `R` on a YouTube page to start recording; press it again to stop and save.
   ...
   - The hotkey only fires when a YouTube page has focus and never triggers inside inputs such as the search or comment box. Conflicts with browser or YouTube player shortcuts are flagged in the settings panel.
...
| `storage` | `session` keeps recording state and the start intent; `sync` keeps the hotkey and crop calibration |

No `host_permissions`, no network requests, no user data collection.
...
- [ ] 20.7 Error cases report correctly: non-watch page (start button disabled), DRM video (warning in the popup after ~2.6 s), capture denied, etc.
