...
## 阶段十一：开始录制前倒计时（默认 3 秒）

> 背景：点「开始录制」的那一刻鼠标往往还压在播放器上、控制条还亮着、或用户还没切全屏，第一帧经常不理想。结论：在**捕获开始之前**插入一段页面倒计时——页面浮层归零 → 先撤浮层 → 再开始捕获，因此倒计时本身绝不会进入成片。

- [ ] 任务 29：倒计时配置与页面浮层
  - [ ] 29.1 `shared/countdown.js` 统一定义配置（`yrCountdownSec`，`storage.sync`，默认 3 秒，范围 0–10；0 = 关闭）
  - [ ] 29.2 `content/countdown.js` 页面浮层：播放器中央的环形进度 + 大数字 + 「取消（Esc）」；归零后淡出 → 移出 DOM → 再等 `SAFE_GAP_MS` 才回报 `YR_COUNTDOWN_DONE`（先撤浮层、后开捕获，绝不入画）
  - [ ] 29.3 浮层期间吞掉页面点击；Esc / 卡片取消按钮 / 再按一次录制快捷键均可放弃本次录制
- [ ] 任务 30：启动链路接入
  - [ ] 30.1 background 新增 `countdown` phase（`YR_COUNTDOWN_START` / `YR_COUNTDOWN_DONE` / `YR_COUNTDOWN_CANCEL` / `YR_CANCEL_COUNTDOWN`），`requestStart()` 统一决定「立即开始 / 先倒计时」
  - [ ] 30.2 `getMediaStreamId()` 只在倒计时结束后申请（streamId 一次性且几秒后过期，提前申请必然失效）
  - [ ] 30.3 兜底：页面脚本不可用 → 降级立即开始；页面消息丢失 → 兜底定时器；标签页关闭 / 跳走 / SW 重启 → 取消或回到空闲
  - [ ] 30.4 popup 倒计时态：剩余秒数 + 「取消倒计时」按钮 + 引导文案；全屏 PiP 小窗同步显示倒计时与「取消倒计时」
  - [ ] 30.5 设置弹窗新增「开始录制倒计时」分组：数字输入 + 0 / 3 / 5 / 10 快捷档位
- [ ] 任务 31：倒计时验收
  - [ ] 31.1 点开始 → 播放器中央出现倒计时圆环并逐秒递减；倒计时期间可切全屏 / 移开鼠标
  - [ ] 31.2 倒计时归零后浮层消失、随即出现录制遮罩；成片首帧不含倒计时浮层
  - [ ] 31.3 取消路径（Esc / 卡片按钮 / popup 按钮 / 再按快捷键）均回到空闲且未产出文件
  - [ ] 31.4 设置改为 0 秒：点击后立即开始（旧行为）；改为 5 / 10 秒：时长正确
  - [ ] 31.5 倒计时途中关闭标签页 / 刷新：不产生残留状态、不误启动录制
